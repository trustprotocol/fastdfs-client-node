...
FdfsClient.prototype.upload = function(file, options) {
    ...
    }).then(async function(tracker) {
        const storage =  await tracker.getStoreStorage(options.group);
        return {storage, tracker}

    }).then(function({storage, tracker}) {
        storage.config.host = tracker.config.host
...
FdfsClient.prototype.download = function(fileId, options) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        const storage = await tracker.getFetchStorage(fileId);
        return {storage, tracker}
    }).then(function({storage, tracker}) {
        storage.config.host = tracker.config.host
...
FdfsClient.prototype.getFileInfo = function(fileId) {
    return this._getTracker().then(async function(tracker) {
        const storage = await tracker.getUpdateStorage(fileId)
        return {storage, tracker};
    }).then(function({storage, tracker}) {
        return storage.getFileInfo(fileId, tracker.config.host);
    });
};
