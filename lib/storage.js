...
Storage.prototype._getConnection = function(host) {
    return this._newConnection(host);
};

Storage.prototype._newConnection = function(host) {
    var _self = this;
    return new Promise(function(resolve, reject) {
        ...
        socket.connect(_self.config.port, host || _self.config.host);
        console.log(_self.config.port, _self.config.host);
 ...
 * @param fileId
 */
Storage.prototype.getFileInfo = function(fileId, host) {
    var charset = this.config.charset;
    return this._getConnection(host).then(function(socket) {
        var gf = helpers.id2gf(fileId);
